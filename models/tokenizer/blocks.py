...
class Encoder(nn.Module):
    def __init__(self, config: EncoderDecoderConfig) -> None:
        ...
        self.conv_in = torch.nn.Conv2d(
            config.in_channels, config.ch, kernel_size=3, stride=1, padding=1
        )
        ...
        self.conv_out = torch.nn.Conv2d(
            block_in, config.z_channels, kernel_size=3, stride=1, padding=1
        )
...
class Decoder(nn.Module):
    def __init__(self, config: EncoderDecoderConfig) -> None:
        ...
        # z to block_in
        self.conv_in = torch.nn.Conv2d(
            config.z_channels, block_in, kernel_size=3, stride=1, padding=1
        )
        ...
        self.norm_out = Normalize(block_in)
        self.conv_out = torch.nn.Conv2d(
            block_in, config.out_ch, kernel_size=3, stride=1, padding=1
        )
